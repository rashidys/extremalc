...
int main(int argc, char** argv) {
    ...
    fprintf(stdout, "Shared memory is opentd with fd: %d\n", shm_fd);
    ...
    char* ptr = (char*)map;
    fprintf(stdout, "The contents of shared memory obhect: %s\n", ptr);
    ...
    if (shm_unlink("/shm0") < 0){
        fprintf(stderr, "ERROR: Unliking shared memory failed: %s\n", strerror(errno));
        return 1;
    }
    return 0;
