...
void* alloc_thread_body(void* arg) {
    shared_array_len = 20;
    shared_array = (char*)malloc(shard_array_len * sizeof(char*));
...
void* printer_thred_body(void* arg) {
    pthread_barrier_wait(&fill_barrier);
    printf(">> %\n", shared_array);
...
int main(int argc, char** argv) {
    ...
    pthtread_barrier_init(&fill_barrier, NULL, 3);
    ...
    res = pthread_create(&odd_filler_thread, &attr, filler_thread_body, &FALSE);
